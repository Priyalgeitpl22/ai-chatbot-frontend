...
export default function DynamicData() {
  ...
  const { dynamicDataList, loading } = useSelector((state: RootState) => state.dynamicData);
  ...
  useEffect(() => {
      ...
      const populatedEntries = dynamicDataList?.map((item: any, index: number) => ({
  ...
  const addNewEntry = () => {
    setNewEntries([...newEntries, { id: Date.now().toString(), prompt: '', apiCurl: '' }]);
  };
  ...
  const updateNewEntry = (id: string, field: 'prompt' | 'apiCurl', value: string) => {
  ...
  const saveAllEntries = async () => {
    const validEntries = newEntries.filter(entry => 
      entry.prompt && entry.apiCurl && entry.prompt.trim() && entry.apiCurl.trim()
    );
    ...
    try {
      ...
      if (allEntriesToCreate.length > 0) {
        await dispatch(createMultipleDynamicData({
          orgId: user?.orgId || '',
      ...
      if (updateCount > 0 && createCount > 0) {
        message = `${updateCount} entr${updateCount > 1 ? 'ies' : 'y'} updated and ${createCount} entr${createCount > 1 ? 'ies' : 'y'} created successfully`;
      } else if (updateCount > 0) {
        message = `${updateCount} entr${updateCount > 1 ? 'ies' : 'y'} updated successfully`;
      } else {
        message = `${createCount} entr${createCount > 1 ? 'ies' : 'y'} created successfully`;
      }
  ...
  const handleDelete = async (dbId: string, localId: string) => {
    if (window.confirm('Are you sure you want to delete this dynamic data entry?')) {
      try {
        ...
        <Box>
            ...
            <Box key={entry.id} sx={{ mb: 3 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                <Typography variant="body2" sx={{ fontWeight: 'bold', color: '#666' }}>
                  Entry {index + 1} {entry.dbId && <span style={{ color: '#999', fontSize: '0.85em' }}>(Existing)</span>}
                </Typography>
                <Box>
                  {entry.dbId && (
                    <IconButton
                      size="small"
                      onClick={() => entry.dbId && handleDelete(entry.dbId, entry.id)}
                      color="error"
                      sx={{ mr: 1 }}
                    >
              ...
              <Grid container spacing={2}>
                <Grid size={12}>
                  <TextField
                    ...
                    fullWidth
                    value={entry.prompt}
                    onChange={(e) => updateNewEntry(entry.id, 'prompt', e.target.value)}
                ...
                <Grid size={12}>
                  <TextField
                    ...
                    fullWidth
                    value={entry.apiCurl}
                    onChange={(e) => updateNewEntry(entry.id, 'apiCurl', e.target.value)}
          ...
          <Button
            ...
            <Button
              ...
              disabled={loading || newEntries.every(e => !e.prompt || !e.apiCurl || !e.prompt.trim() || !e.apiCurl.trim())}
              sx={{ textTransform: 'none', px: 4 }}
            >
              {loading ? (
                <CircularProgress size={20} color="inherit" />
              ) : (
                `Save All (${newEntries.filter(e => e.prompt && e.apiCurl && e.prompt.trim() && e.apiCurl.trim()).length})`
              )}
            </Button>
          </Box>
        </Box>
